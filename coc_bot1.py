...
def numberOCR(region: tuple, ocrType: str) -> int:
    ...
    numberImages = {
        'opponentLoot': [f"oppLoot_{i}.png" for i in range(10)]
    }.get(ocrType, [])
    
    resultList = []
    try:
        for i, img in enumerate(numberImages):
            logging.debug(f"Searching for digit {i} ({img})")
            for match in pyautogui.locateAllOnScreen(img, confidence=0.8, region=region):
                logging.debug(f"Found digit {i} at {match}")
                resultList.append((match, i))
        
        sortedResult = sorted(resultList, key=lambda x: x[0][0])
        logging.debug(f"Sorted OCR results: {sortedResult}")
        result = sum(10**(len(sortedResult) - i - 1) * val for i, (_, val) in enumerate(sortedResult))
